jQuery(function() {
	...
	var defaultFontSize = parseFloat(jQuery('body').css('font-size'));
	var fontSize = defaultFontSize;

	jQuery('a.increase-font').click(function() {
		jQuery('body').css('font-size', ++fontSize);
	});

	jQuery('a.decrease-font').click(function() {
		if (fontSize > defaultFontSize) {
			jQuery('body').css('font-size', --fontSize);
		}
	});

	jQuery('a.resize-font').click(function() {
	...
	jQuery('a.toggle-contraste').click(function() {
		if (!jQuery('div.layout').hasClass('contraste')) {
			...
			if (layout_classes != 'undefined') layout_classes = layout_classes + ' contraste';
	...
	jQuery('a.mainmenu-toggle').click(function() {
		if (!jQuery('#navigation-section').is(':visible')) {
			...
			if (jQuery(document).width() > 767) jQuery('#em-destaque').fadeOut();
		} else {
			jQuery('#navigation-section').slideUp();
			if (jQuery(document).width() > 767) jQuery('#em-destaque').fadeIn();
		}
	...
	jQuery('#navigation-section > nav > h2 > i:not(.visible-tablet)')
		.parent()
		.css('cursor', 'pointer');
	URL = document.URL;
...
function resize() {
	...
	if (jQuery(document).width() < 979) {
		...
		if (jQuery('#navigation-section').is(':visible')) jQuery('#navigation-section').hide();

		if (!jQuery('#em-destaque').is(':visible')) jQuery('#em-destaque').fadeIn();
	} else {
		jQuery('#navigation-section nav.closed h2 i')
			.removeClass('icon-chevron-up')
			.addClass('icon-chevron-down');
		if (jQuery('body').hasClass('responsivo-menor-979')) {
			jQuery('body').removeClass('responsivo-menor-979');
			jQuery('#navigation nav:not(.closed) ul').show();
			jQuery('#navigation nav:not(.closed) h2 i')
...
function browser_adjusts() {
	if (
		...
		jQuery('#portal-searchbox .searchField').val(
			jQuery('#portal-searchbox .searchField').attr('title')
		);
		jQuery('#portal-searchbox .searchField').focus(function() {
			if (jQuery(this).val() == jQuery('#portal-searchbox .searchField').attr('title'))
				jQuery(this).val('');
		});
		jQuery('#portal-searchbox .searchField').blur(function() {
			if (jQuery(this).val() == '')
				jQuery(this).val(jQuery('#portal-searchbox .searchField').attr('title'));
		});
	}
}
...
function carousel_addons() {
	...
	jQuery('.galeria-thumbs .galeria-image')
		.eq(index)
		.addClass('active');
	jQuery('.galeria-thumbs .galeria-image')
		.children('a')
