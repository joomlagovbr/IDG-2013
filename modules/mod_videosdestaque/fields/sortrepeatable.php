...
class JFormFieldSortrepeatable extends JFormFieldRepeatable
{
	...
	protected function getInput()
	{
		...
		$head_row_str[] = '<th></th>';
		$body_row_str[] = '<td class="order nowrap center hidden-phone" style="width: 32px;">'
							.'<span class="sortable-handler" style="cursor: move;"><i class="icon-menu"></i></span>'
							.'<input type="text" class="width-20 text-area-order" value="0" size="5" name="order[]" style="display:none"></td>';
		...
		$table = '<table id="' . $this->id . '_table" class="adminlist ' . $this->element['class'] . ' table table-striped">'
					. '<thead><tr>'
					. '<td colspan="'.count($head_row_str).'">Edição de itens de vídeo (até 3 itens)'
					. '<button class="close closebutton close-modal" style="float:right" type="button">×</button></td>'
					. '</tr><tr>' . implode("\n", $head_row_str) . '</tr></thead>'
					. '<tbody><tr class="dndlist-sortable">' . implode("\n", $body_row_str) . '</tr></tbody>'
				. '</table>';
